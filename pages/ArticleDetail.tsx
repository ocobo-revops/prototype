    ...
    <div className="w-full bg-white pt-32 pb-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12 flex items-center justify-between">
          <Link to="/resources" className="inline-flex items-center text-gray-400 hover:text-semantic-text font-black uppercase tracking-widest text-micro transition-colors">
            <ArrowLeft size={14} className="mr-2" /> Retour au blog
          </Link>
          <div className="flex items-center gap-4 text-gray-300">
            <Share2 size={16} />
            <span className="h-4 w-px bg-gray-200"></span>
            <button className="hover:text-semantic-text transition-colors"><Linkedin size={16} /></button>
            <button className="hover:text-semantic-text transition-colors"><Twitter size={16} /></button>
          </div>
        </div>

        {/* HERO ARTICLE */}
        <div className="max-w-4xl mx-auto text-center mb-20">
          <span className="font-display font-bold text-semantic-accent bg-ocobo-coral-light px-4 py-1.5 text-micro uppercase tracking-[0.3em] mb-8 inline-block border border-ocobo-coral/20">
            {article.category}
          </span>
          <h1 className="font-display text-4xl md:text-6xl font-black text-semantic-text mb-10 leading-[1.05] tracking-tight">
            {article.title}
          </h1>
          <div className="flex flex-wrap items-center justify-center gap-8 text-sm font-bold text-gray-400 uppercase tracking-widest">
            <div className="flex items-center gap-2">
                <Calendar size={14} className="text-semantic-primary" />
                <span>{article.date}</span>
            </div>
            <div className="flex items-center gap-2">
                <Clock size={14} className="text-semantic-info" />
        ...
        <div className="flex flex-col lg:flex-row gap-16">
          ...
          <aside className="lg:w-1/4">
            <div className="sticky top-32">
              <h4 className="font-display font-black text-micro uppercase tracking-[0.4em] text-semantic-text mb-8">Sommaire</h4>
                ...
                {article.sections.map((section) => (
                  <a 
                    ...
                    className={`block text-sm font-medium transition-all duration-300 border-l-2 pl-4 ${
                      ...
                      ? 'border-ocobo-yellow text-semantic-text translate-x-1' 
              ...
              <div className="mt-20 p-8 bg-semantic-text rounded-2xl text-white relative overflow-hidden">
                  ...
                  <Link to="/contact">
                      <button className="w-full py-3 bg-semantic-primary text-semantic-text font-bold text-xs uppercase tracking-widest rounded-lg hover:bg-white transition-colors">Discuter avec nous</button>
                  </Link>
              </div>
            </div>
          </aside>
          ...
          <article className="lg:w-3/4 max-w-3xl">
                      ...
                      <ul className="space-y-4 mt-8">
                        {section.list.map((item, i) => (
                          <li key={i} className="flex items-start gap-4 p-5 bg-gray-50 rounded-xl border border-transparent hover:border-ocobo-yellow/20 transition-all">
                            <div className="mt-1 text-semantic-primary">
            ...
            <div className="mt-24 pt-12 border-t border-gray-100 flex flex-col md:flex-row items-center gap-8 bg-gray-50/50 p-10 rounded-3xl">
                ...
                <div className="text-center md:text-left">
                    <h4 className="font-display text-2xl font-bold text-semantic-text mb-1">{article.author.name}</h4>
            ...
            <div className="mt-20 bg-semantic-primary p-10 md:p-14 rounded-xl relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4 opacity-5">
                    <Layout size={200} strokeWidth={1} />
                </div>
                <div className="relative z-10 max-w-xl">
                    <h3 className="font-display text-3xl font-black text-semantic-text mb-4 italic">Le RevOps vous passionne ?</h3>
                    <p className="text-semantic-text/70 font-bold text-lg mb-8">
                    ...
                    <form className="flex flex-col sm:flex-row gap-4">
                        <input 
                            ...
                            className="flex-grow px-6 py-4 bg-white/90 focus:bg-white border-none outline-none text-semantic-text font-bold placeholder-gray-400 rounded-xl"
                        />
                        <button className="px-8 py-4 bg-semantic-text text-white font-black uppercase tracking-widest text-micro rounded-xl hover:bg-black transition-all">S'inscrire</button>
                    </form>
                </div>
            </div>
          </article>
        ...
        <div className="mt-32 pt-20 border-t border-gray-100">
            <h3 className="font-display text-3xl font-black text-semantic-text mb-12">À lire ensuite</h3>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                {[1, 2, 3].map((i) => (
                    <div key={i} className="group cursor-pointer">
                        ...
                        <span className="font-display font-bold text-micro uppercase tracking-widest text-semantic-accent mb-3 block">Performance</span>
                        <h4 className="font-display text-xl font-bold text-semantic-text mb-4 group-hover:text-semantic-primary transition-colors leading-tight">
